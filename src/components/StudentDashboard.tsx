...
import { Flame, BookOpen, Clock, ChevronRight, Sparkles } from 'lucide-react';
...
export const StudentDashboard: React.FC<StudentDashboardProps> = ({
  ...
  // En son okunan kitap (devam et)
  const continueBook = useMemo(() => {
    const started = books.filter((b) => (b.progressPercent || 0) > 0 && (b.progressPercent || 0) < 100);
  ...
  const finishedCount = useMemo(
    () => books.filter((b) => (b.progressPercent || 0) >= 100).length,
    [books]
  );
        ...
        <div className="bg-white rounded-2xl p-4 border border-[#e6e8ea] shadow-xs flex flex-col items-center justify-center text-center relative overflow-hidden group hover:border-emerald-200 transition-all">
          ...
          <div className="w-full bg-slate-100 h-1 rounded-full mt-2.5 overflow-hidden">
            <div
              ...
              style={{ width: `${books.length > 0 ? (finishedCount / books.length) * 100 : 0}%` }}
            ></div>
          </div>
        </div>
      </div>
                ...
                <div className="p-3 flex-1 flex flex-col justify-between">
                  ...
                  <div className="w-full bg-slate-100 h-1 rounded-full mt-3 overflow-hidden">
                    <div
                      ...
                      style={{ width: `${Math.max(book.progressPercent || 0, 5)}%` }}
                    ></div>
                  </div>
                </div>
              </div>
